...
namespace ex10_MovieFinder2024
{
    ...
    public partial class MainWindow : MetroWindow
    {
        ...
        {
            string tmdb_apiKey = "1aee496c40c67b8be663601b50f17fb8"; //TMDB 사이트에서 제공받은 API키.
            ...
            {
                // tmdb api 요청 
                req = WebRequest.Create(openApiUri);    // URL 을 넣어서 객체를 생성   요청 함 
            ...
            //result string을 json 으로 변경
            var jsonResult = JObject.Parse(result);  // type. Parse(String) 타입으로 변경
            ...
            var jsonArrayy = results as JArray; //results 가 json 배열이기 때문에 jarry 를 사용해서 쓰게 됨 jarry 는 list와 동일해서 foreach 사용가능


            var movieItems = new List<MovieItem>();
            foreach (var item in jsonArrayy)
            {
        ...
        private async void GrdResult_SelectedCellsChanged(object sender, SelectedCellsChangedEventArgs e)
        {
            ...
            {
                Debug.WriteLine($"{ex.Message}");  //  try catch 치면서 어벤져스 검색후에 스타워즈를 쳐도 안꺼지고 실행 , 없으면 렉걸리면서 꺼짐
            }
        }
        ...
        private async void BtnDelFavorite_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                using(SqlConnection conn = new SqlConnection(Helpers.Common.CONNSTRING))
                {
                    ...
                    {
                        await this.ShowMessageAsync("삭제", $"즐겨찾기{GrdResult.SelectedItems.Count}건종 {delRes}건 삭제");
                    }
